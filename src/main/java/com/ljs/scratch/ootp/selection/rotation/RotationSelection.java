...
public final class RotationSelection
{
    ...
    private RotationSelection(TeamStats predictions, PitcherOverall method, RotationDefinition definition)
    {
    ...
    public Rotation select(Iterable available)
    {
        ...
        Iterator i$ = byOverall().compound(method.byWeightedRating()).compound(Player.byAge()).sortedCopy(Selections.onlyPitchers(available)).iterator();
        do
        {
            if(!i$.hasNext())
                break;
            Player p = (Player)i$.next();
            if(Slot.getPlayerSlots(p).contains(Slot.SP) && starters.size() < definition.getRotationSize().intValue())
                starters.add(p);
        } while(true);
        rotation.setStarters(starters);
        List relievers = byOverall().compound(method.byWeightedRating()).compound(Player.byAge()).sortedCopy(Selections.onlyPitchers(Sets.difference(ImmutableSet.copyOf(available), ImmutableSet.copyOf(starters)))).subList(0, definition.getRelieversSize().intValue());
    ...
    public static RotationSelection regularSeason(TeamStats pitching, PitcherOverall method)
    {
        return new RotationSelection(pitching, method, RotationDefinition.regularSeason());
    }

    public static RotationSelection playoffs(TeamStats pitching, PitcherOverall method)
    {
        return new RotationSelection(pitching, method, RotationDefinition.playoffs());
    }
