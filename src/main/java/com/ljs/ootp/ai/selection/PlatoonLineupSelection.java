 ...
 * @author lstephen
 */
public class PlatoonLineupSelection implements Selection {
    ...
    public PlatoonLineupSelection(Multiset<Slot> slots, Function<Player, Integer> value, TeamStats<BattingStats> predictions, SplitPercentages splits) {
        this.slots = slots;
        this.value = value;
    ...
    public ImmutableMultimap<Slot, Player> select(
        ...
        return new PrioritzeStarterSelection(slots, value, predictions, splits).select(Iterables.concat(forced, best.getVsRhp().playerSet()), available);
    }
}
