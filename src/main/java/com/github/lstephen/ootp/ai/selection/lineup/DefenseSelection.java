...
public class DefenseSelection {
  ...
  private Defense select(ImmutableSet<Player> selected) {
    if (!CACHE.containsKey(selected)) {
      HillClimbing<Defense> hc =
          HillClimbing.<Defense>builder()
      ...
      CACHE.put(
          selected,
          new RepeatedHillClimbing<Defense>(Defense.randomGenerator(selected), hc).search());
    }
    return CACHE.get(selected);
  }

  private Ordering<Defense> heuristic() {
    return Defense.byScore().compound(Defense.byAge().reverse());
  }
