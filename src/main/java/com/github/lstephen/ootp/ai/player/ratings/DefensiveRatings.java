...
public class DefensiveRatings {
  ...
  private static final ImmutableMap<Position, FieldingRatings.Weighting> WEIGHTINGS =
      ImmutableMap.<Position, FieldingRatings.Weighting>builder()
          ...
          .put(
              Position.FIRST_BASE,
              FieldingRatings.weighting().range(1.0).errors(1.25).arm(.75).dp(1.0))
          .put(
              Position.SECOND_BASE,
              FieldingRatings.weighting().range(1.25).errors(1.25).arm(.25).dp(1.25))
          .put(
              Position.THIRD_BASE,
              FieldingRatings.weighting().range(1.25).errors(1.0).arm(1.5).dp(.25))
          .put(
              Position.SHORTSTOP,
              FieldingRatings.weighting().range(1.0).errors(1.0).arm(1.0).dp(1.0))
    ...
    switch (p) {
      ...
      case FIRST_BASE:
        if (infield != null && outfield != null) {
          skill = Math.max(infield.score(WEIGHTINGS.get(p)), outfield.score(WEIGHTINGS.get(p)));
        }
        break;
      default:
    }

    Double score = skill == null ? rating : ((skill + 1.25 * rating) / 2.25);
