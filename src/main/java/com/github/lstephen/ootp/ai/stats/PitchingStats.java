...
public class PitchingStats implements Stats<PitchingStats> {
  ...
  @JsonIgnore private PitchingStats leaguePitching;
  ...
  public double getTriplesPerPlateAppearance() {
    // System.out.println(triples + "/" + getPlateAppearances() + "=" +
    // perPlateAppearance(triples));
  ...
  public double getBabip() {
    if (atBats - strikeouts - homeRuns == 0) {
      ...
      return (double) (hits - homeRuns) / (atBats - strikeouts - homeRuns);
    }
  }
  ...
  public double getFip() {
    return (double) (13 * homeRuns + 3 * walks - 2 * strikeouts) / getInningsPitched()
        + FIP_FUDGE_FACTOR;
  }
