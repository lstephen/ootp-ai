...
public enum Slot {
  ...
  public static final Multiset<Slot> MAJOR_LEAGUE_PITCHING_SLOTS =
  ...
  private static ImmutableList<Slot> getPitcherSlots(Player p) {
    return p.getPitchingRatings().getVsLeft().getEndurance() <= 5
        ? ImmutableList.of(MR, P)
        : ImmutableList.of(SP, P);
  }
  ...
  private static ImmutableList<Slot> getBatterSlots(Player p) {
    ...
    List<Position> specialtyPositions =
        ImmutableList.copyOf(
            Iterables.filter(
                Ordering.natural()
                    ...
                    .sortedCopy(
                        ImmutableSet.of(
                            Position.CATCHER, Position.SHORTSTOP, Position.CENTER_FIELD)),
    ...
    double ifScore =
        ((Double)
                Ordering.natural()
                    .max(
                        def.getPositionScore(Position.SECOND_BASE),
                        def.getPositionScore(Position.THIRD_BASE),
                        def.getPositionScore(Position.SHORTSTOP),
                        new Double[0]))
            .doubleValue();
    double ofScore =
        ((Double)
                Ordering.natural()
                    .max(
                        def.getPositionScore(Position.LEFT_FIELD),
                        def.getPositionScore(Position.CENTER_FIELD),
                        def.getPositionScore(Position.RIGHT_FIELD),
                        new Double[0]))
            .doubleValue();
