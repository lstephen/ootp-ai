...
public class DepthChart {
  ...
  public void print(PrintWriter w) {
    for (Position p :
        Ordering.explicit(Arrays.asList(Position.values())).sortedCopy(starters.keySet())) {
    ...
    public static Function<Backup, String> format() {
      return new Function<Backup, String>() {
        ...
        public String apply(Backup bu) {
          ...
          return String.format("(%2d) %-15s", bu.percentage, bu.player.getShortName());
        }
      };
    }

    public static Backup create(Position position, Player player, Integer percentage) {
