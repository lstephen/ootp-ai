...
public class PlayerValue {
  ...
  public Integer getNowValue(Player p) {
    Double defense =
        Selections.isHitter(p) ? PlayerDefenseScore$.MODULE$.atBestPosition(p, true).score() : 0.0;
  ...
  public Integer getNowValue(Player ply, Position pos) {
    ...
    Double defense = Selections.isHitter(ply) ? new PlayerDefenseScore(ply, pos).score() : 0.0;
  ...
  public Integer getFutureValue(Player p) {
    ...
    Double risk = ceiling > now ? Math.pow((double) (ceiling - now) / ceiling, 2) : 0.0;
