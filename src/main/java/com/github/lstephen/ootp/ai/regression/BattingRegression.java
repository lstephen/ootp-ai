...
public final class BattingRegression {
  ...
  private void addData(BattingStats stats, BattingRatings<?> ratings) {
    for (int i = 0; i < stats.getPlateAppearances(); i++) {
      getRegression(Predicting.HITS).addBattingData(ratings, stats.getHitsPerPlateAppearance());
      getRegression(Predicting.DOUBLES)
          .addBattingData(ratings, stats.getDoublesPerPlateAppearance());
      getRegression(Predicting.TRIPLES)
          .addBattingData(ratings, stats.getTriplesPerPlateAppearance());

      getRegression(Predicting.HOME_RUNS)
          .addBattingData(ratings, stats.getHomeRunsPerPlateAppearance());
  ...
  private BattingStats predict(BattingRatings<?> ratings, Long plateAppearances) {
    ...
    long predictedKs =
        ratings.getK().isPresent()
            ? Math.round(plateAppearances * predict(Predicting.KS, ratings))
            : 0;
  ...
  private void runRegression(Site site) {
    ...
    Iterable<TeamStats<BattingStats>> historical = history.loadBatting(site, currentSeason, 5);

    for (TeamStats<BattingStats> h : historical) {
      addData(h);
    }

    Iterable<TeamStats<BattingStats>> all = Iterables.concat(ImmutableList.of(stats), historical);
