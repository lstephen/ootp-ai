...
public final class DiskCachingLoader implements PageLoader {
  ...
  private static final class DiskCache extends AbstractCache<String, Document> {
    ...
    private File getFileForKey(Object key) {
      ...
      String pathName = StringUtils.substringBeforeLast(key.toString(), "/");

      try {
        File path = new File(dir, URLEncoder.encode(pathName, CHARSET.name()));

        String fileName = StringUtils.substringAfterLast(key.toString(), "/");

        File forKey = new File(path, URLEncoder.encode(fileName, CHARSET.name()));
