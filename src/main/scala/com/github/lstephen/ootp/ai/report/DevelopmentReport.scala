...
class DevelopmentReport(site: Site, implicit val predictor: Predictor) extends Printable {
  ...
  def print[T: Regressable](pds: Seq[PlayerDevelopment], filter: PlayerDevelopment => Boolean, regressedOn: Player => Splits[T])(implicit w: PrintWriter): Unit = {
  ...
  def print(hitting: Seq[PlayerDevelopment], pitching: Seq[PlayerDevelopment])(implicit w: PrintWriter): Unit = {
    w.println("Hitters")
  ...
  def print(w: PrintWriter): Unit = {
    ...
    val dHitting = PlayerDevelopment.betweenTeamStats((fromHitting, fromPredictions), (site.getTeamBatting, predictor))
    val dPitching = PlayerDevelopment.betweenTeamStats((fromPitching, fromPredictions), (site.getTeamPitching, predictor))

    val draftHitting = PlayerDevelopment.betweenPlayers((draft.getPlayers.asScala.toSeq, draftPredictions), (site.getTeamBatting.getAllRatings.asScala.toSeq, predictor))
    val draftPitching = PlayerDevelopment.betweenPlayers((draft.getPlayers.asScala.toSeq, draftPredictions), (site.getTeamPitching.getAllRatings.asScala.toSeq, predictor))
...
class PlayerDevelopment(from: (Player, Predictor), to: (Player, Predictor)) extends Scoreable {

  def this(pid: PlayerId, from: (TeamStats[_], Predictor), to: (TeamStats[_], Predictor)) =
...
object PlayerDevelopment {
  ...
  def betweenTeamStats(from: (TeamStats[_], Predictor), to: (TeamStats[_], Predictor)): Seq[PlayerDevelopment] =
    betweenPlayers((from._1.getAllRatings.asScala.toSeq, from._2), (to._1.getAllRatings.asScala.toSeq, to._2))

  def betweenPlayers(from: (Seq[Player], Predictor), to: (Seq[Player], Predictor)): Seq[PlayerDevelopment] = {
    val pids = playerIdSet(from._1).intersect(playerIdSet(to._1))

    pids.map(pid => new PlayerDevelopment((from._1.find(_.getId == pid).get, from._2), (to._1.find(_.getId == pid).get, to._2))).toList
  }
