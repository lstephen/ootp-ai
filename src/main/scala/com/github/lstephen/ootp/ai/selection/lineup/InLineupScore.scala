...
object InLineupScore {
  ...
  def apply(ply: Player, pos: Position)(implicit ps: Predictor): InLineupScore = {
    new InLineupScore(ply, pos, None)
  }

  def apply(ply: Player, pos: Position, vs: VsHand)(implicit ps: Predictor): InLineupScore = {
    new InLineupScore(ply, pos, Some(vs))
  }

  def sort(players: Traversable[Player], pos: Position, vs: VsHand)(implicit ps: Predictor): Seq[Player] = {
