...
class ReplacementLevels(levels: Map[Position, Score])(implicit ps: Predictions) extends Printable {
  val pv = new PlayerValue(ps, null, null)

  def get(pos: Position): Score =
    levels.get(pos).getOrElse(throw new IllegalStateException())

   def get(ply: Player, pos: Position): Score =
     Score(pv.getNowValue(ply, pos).intValue) - get(pos)
...
object ReplacementLevels {
  ...
  def getForNow(implicit ps: Predictions): ReplacementLevels =
    _forNow.getOrElse {
      _forNow = Some(getFor(Context.newRoster.getOrElse(shouldNotHappen)))
      _forNow getOrElse shouldNotHappen
    }

  def getFor(r: Roster)(implicit ps: Predictions): ReplacementLevels = {
    val pv = new PlayerValue(ps, null, null)

    new ReplacementLevels(
      (Position.hitting ++ Position.pitching).foldLeft(Map.empty[Position, Score])(
        (m, pos) => m + (pos -> Score(top(r)(pv.getNowValue(_, pos).intValue)))
      ))
  }

  def top[S](r: Roster)(f: Player => S)(implicit ps: Predictions, ord: Ordering[S]): S =
    r.getMinorLeaguers
      .toList
      .map(f)
      .sorted
